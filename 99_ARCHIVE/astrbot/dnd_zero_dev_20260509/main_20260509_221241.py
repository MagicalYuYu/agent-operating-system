...
如需更多帮助，请联系插件管理员。
    ...
    @dnd.command("menu")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_menu_alias(self, event: AstrMessageEvent):
        """菜单（别名）"""
        for handler in self.__class__.dnd_menu.__wrapped__.__code__.co_varnames:
            pass
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("帮助")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_help_cn(self, event: AstrMessageEvent):
        """获取帮助信息（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("掷骰")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_roll_cn(self, event: AstrMessageEvent, dice: str = "d20"):
        """掷骰（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("先攻")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_initiative_cn(self, event: AstrMessageEvent):
        """先攻掷骰（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("创建")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_create_cn(self, event: AstrMessageEvent):
        """创建角色（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("状态")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_status_cn(self, event: AstrMessageEvent):
        """查看角色状态（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("属性")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_abilities_cn(self, event: AstrMessageEvent):
        """查看属性面板（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("技能")
    async def dnd_skill_cn(self, event: AstrMessageEvent, *, skill: str = ""):
        """技能检定（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("豁免")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_save_cn(self, event: AstrMessageEvent, *, ability: str = ""):
        """豁免检定（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("攻击")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_attack_cn(self, event: AstrMessageEvent, *, target: str = "敌人"):
        """攻击检定（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("伤害")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_damage_cn(self, event: AstrMessageEvent, *, dice: str = "1d6"):
        """伤害掷骰（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("法术列表")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_spells_cn(self, event: AstrMessageEvent):
        """查看已知法术（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("法术")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_spell_cn(self, event: AstrMessageEvent, *, spell_name: str = ""):
        """查询法术信息（中文）"""
        if not self._check_group_permission(event):
            yield event.plain_result("⚠️ 此群组未被授权使用 DND 插件")
            event.stop_event()
    ...
    @dnd.command("名言")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_quote_cn(self, event: AstrMessageEvent):
        """随机DND名言（中文）"""
        quotes = [
            "「在这个世界上，只有死亡是确定无疑的。」 - 《死亡公主》",
            "「不要相信任何人，尤其是那些看起来友善的人。」 - 某位老练的冒险者",
...
数据来源: 5etools中文站
    ...
    @dnd.command("管理员")
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_admin_cn(self, event: AstrMessageEvent, action: str = ""):
        """管理员指令（中文）"""
        if not self._check_superadmin(event):
            yield event.plain_result("⚠️ 此指令仅限超级管理员使用")
            event.stop_event()
...
数据来源: 5etools中文站
    ...
    def _get_ability_score(self, char: Character, ability_key: str) -> int:
        """获取属性值"""
        ability_map = {
            "strength": char.abilities.strength,
            "dexterity": char.abilities.dexterity,
