...
3. 切换到 **CODE 模式**（Trae 右上角或左侧面板切换）
4. Trae 会自动读取根目录的 `AGENTS.md`，AOS 的规则即开始生效

**其他平台（Claude Code / Codex 等）**

将 AOS 目录设为对应平台的工作区根目录，参照 [03_TOOLS/adapters/](03_TOOLS/adapters/) 中的适配模板完成配置。AOS 规则体系基于 Trae CODE 模式设计，其他平台需自行完成适配调整。

### 3. 配置 Trae Rules

AOS 需要 3 条 Trae Rules 才能完整运行。配置路径：**Trae 设置 → 规则与记忆 → 规则 → 本地 → 点击「+」添加规则**
...
每条规则的具体内容见 [Trae 配置指南](docs/trae-setup-guide.md)，文档包含完整的操作步骤和规则全文，可直接复制粘贴。

### 4. 配置自定义命令（可选）

AOS 提供 3 条自定义命令，配置路径：**Trae 设置 → 规则与记忆 → 命令 → 点击「+」添加命令**
...
每条命令的具体内容见 [Trae 配置指南](docs/trae-setup-guide.md)。
...
AOS 1.0 对 **Trae** 适配性最优（规则体系、Skill 机制均基于 Trae CODE 模式设计），同时也支持 **Claude Code** 和 **Codex**。用户下载部署后，将 AOS 目录设为对应平台的根目录即可快速完成适配。
