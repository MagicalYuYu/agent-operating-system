...
📜 **基础指令：**
  /dnd menu /dnd 帮助      - 显示本菜单
  /dnd roll <骰子>         - 掷骰（如 d20, 2d6+3）
  /dnd init /dnd 先攻       - 先攻掷骰

👤 **角色系统：**
  /dnd create /dnd 创建     - 创建角色
  /dnd status /dnd 状态     - 查看角色状态
  /dnd abilities /dnd 属性   - 查看属性面板

🎯 **检定系统：**
  /dnd skill /dnd 技能      - 技能检定
  /dnd save /dnd 豁免       - 豁免检定

⚔️ **战斗系统：**
  /dnd attack /dnd 攻击     - 攻击检定
  /dnd dmg /dnd 伤害       - 伤害掷骰

📖 **法术系统：**
  /dnd spell /dnd 法术     - 查询法术信息
  /dnd spells /dnd 法术列表 - 查看法术列表

🎭 **娱乐系统：**
  /dnd quote /dnd 名言      - 随机DND名言

🔧 **管理员指令：**
  /dnd admin /dnd 管理员    - 管理员操作
...
数据来源: 5etools中文站 | 遵循 Creative Commons 许可证
    ...
    async def dnd_spell(self, event: AstrMessageEvent, *, spell_name: str = ""):
        ...
        if not spell_name:
            yield event.plain_result("请提供法术名称，如: /dnd spell 火球")
            return
