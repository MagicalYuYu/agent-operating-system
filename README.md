...
> 一个用于管理 Clash 代理规则的 Chrome 扩展，致敬 SwitchyOmega / ZeroOmega。

| 维度 | 详情 |
|:-----|:-----|
| **项目定位** | Chrome Extension + Native Messaging Host，三模式代理切换 + 域名规则管理 |
| **技术栈** | Manifest V3 + Vanilla JS + PowerShell + Material Design 3 |
| **版本量级** | v1.3.3 · 89 commits · 4 releases · MIT License |
| **AOS 角色** | v1.3.0 开发全程基于 AOS 框架协作完成 |
| **零侵入证明** | AOS 仅作为开发期协作框架，运行时 ClashOmega 保持 0 Dependencies |

**AOS 在其中的价值**：

- 🧠 **多会话状态保持**：管理"扩展前端 + Native Host 后端 + 安装脚本"多模块协同开发，AI 在不同对话中保持上下文一致
- ✅ **Maker/Checker 分离**：保障 v1.3.0 一次性修复 13 项安全漏洞（XSS / 路径遍历 / 进程注入等）的严谨性
- 📁 **文件系统规范**：让浏览器扩展、Native Host、官网落地页三个子项目井然有序，避免文件混杂

> *"本项目基于 AOS（Agent Operating System）框架协作完成。"*
> —— ClashOmega README 致谢章节

ClashOmega 证明了 AOS 能支撑完整产品级项目的开发：从架构设计到安全加固，从多模块协同到版本迭代，AOS 作为开发期协作框架全程参与，而产品本身保持零运行时依赖。
