...
它不是给 AI 加个插件，而是为 AI 装上一整套操作系统：有内核规则、有文件系统、有桌面界面、有应用商店、有持久记忆。AI 在这个系统里运行，就像程序在操作系统里运行——有规可循、有迹可查、有态可存。
...
内置 `aos_sync_github.py` 脚本，白名单模式过滤文件 + 自动内容脱敏，安全地将 AOS 框架同步到 GitHub 上传目录，避免敏感数据泄漏。
...
| 工具脚本 | [03_TOOLS/scripts/](03_TOOLS/scripts/) | 系统工具：自检、数据生成、迁移、GitHub 同步 |
