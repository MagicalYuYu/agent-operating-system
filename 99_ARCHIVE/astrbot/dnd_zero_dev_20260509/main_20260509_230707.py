...
━━━ 💡 使用说明 ━━━
1. 首先使用 /dnd 创建 创建你的角色
2. 使用 /dnd 状态 查看角色信息
3. 使用 /dnd 掷骰 开始掷骰冒险

数据来源: 5etools中文站 | Creative Commons 许可证
        """.strip()
        return help_text
    ...
    @staticmethod
    def _get_ability_score(char: Character, ability_key: str) -> int:
        """获取属性值"""
        ability_map = {
            "strength": char.abilities.strength,
            "dexterity": char.abilities.dexterity,
        ...
        return ability_map.get(ability_key, 10)

    @staticmethod
    def _get_ability_name_cn(ability_key: str) -> str:
        """获取属性中文名"""
        ability_cn_map = {
            "strength": "力量",
            "dexterity": "敏捷",
        ...
        return ability_cn_map.get(ability_key, ability_key)

    @staticmethod
    def _get_ability_emoji(ability_key: str) -> str:
        """获取属性对应emoji"""
        emoji_map = {
            "strength": "💪",
            "dexterity": "🏃",
        ...
        return emoji_map.get(ability_key, "📊")
    ...
    async def dnd_initiative(self, event: AstrMessageEvent):
        ...
        if user_id in self.characters:
            ...
            dex_score = char.abilities.dexterity
        ...
        if user_id in self.characters:
            result_text += f" + 敏捷调整{self._format_mod(dex_mod)} (敏捷值{dex_score})"
        result_text += f"\n━━━━━━━━━━━━━━━━━━━━\n"
    ...
    @dnd.command("abilities", alias={"属性"})
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_abilities(self, event: AstrMessageEvent):
        ...
        abilities_text += f"━━━━━━━━━━━━━━━━━━━━\n"
        abilities_text += f"{self._get_ability_emoji('strength')} {self._get_ability_name_cn('strength')}: {char.abilities.strength} ({self._format_mod(mods['strength'])})\n"
        abilities_text += f"{self._get_ability_emoji('dexterity')} {self._get_ability_name_cn('dexterity')}: {char.abilities.dexterity} ({self._format_mod(mods['dexterity'])})\n"
        abilities_text += f"{self._get_ability_emoji('constitution')} {self._get_ability_name_cn('constitution')}: {char.abilities.constitution} ({self._format_mod(mods['constitution'])})\n"
        abilities_text += f"{self._get_ability_emoji('intelligence')} {self._get_ability_name_cn('intelligence')}: {char.abilities.intelligence} ({self._format_mod(mods['intelligence'])})\n"
        abilities_text += f"{self._get_ability_emoji('wisdom')} {self._get_ability_name_cn('wisdom')}: {char.abilities.wisdom} ({self._format_mod(mods['wisdom'])})\n"
        abilities_text += f"{self._get_ability_emoji('charisma')} {self._get_ability_name_cn('charisma')}: {char.abilities.charisma} ({self._format_mod(mods['charisma'])})\n"
        abilities_text += f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    async def dnd_skill(self, event: AstrMessageEvent, *, skill: str = ""):
        ...
        if not skill:
            skills_text = "🎯 技能列表 (18项技能)\n"
            skills_text += "━━━━━━━━━━━━━━━━━━━━\n"
            skills_text += "💪 力量系: 运动\n"
            skills_text += "🏃 敏捷系: 杂技、巧手、隐匿\n"
            skills_text += "📚 智力系: 奥秘、历史、调查、自然、宗教\n"
            skills_text += "👁 感知系: 驯兽、洞察、医药、察觉、求生\n"
            skills_text += "✨ 魅力系: 欺瞒、威吓、表演、说服\n"
            skills_text += "\n💡 使用 /dnd 技能 <名称> 进行检定\n例如: /dnd 技能 运动"
        ...
        result_text += f"━━━━━━━━━━━━━━━━━━━━\n"
        result_text += f"🎲 D20: {result.roll}\n"
        result_text += f"📊 属性调整: {self._format_mod(result.ability_modifier)} (属性值{self._get_ability_score(char, result.ability_name)})\n"
        if result.proficiency_bonus > 0:
            result_text += f"⚔️ 熟练加值: +{result.proficiency_bonus}\n"
        result_text += f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    async def dnd_save(self, event: AstrMessageEvent, *, ability: str = ""):
        ...
        if not ability:
            yield event.plain_result(f"🛡️ 豁免检定\n\n用法: /dnd 豁免 <属性>\n可选: str(力量), dex(敏捷), con(体质), int(智力), wis(感知), cha(魅力)\n或直接使用中文: 力量、敏捷、体质、智力、感知、魅力")
            return
        ...
        if user_id in self.characters:
            ...
            ability_score = self._get_ability_score(char, ability_key)
            result = DiceRoller.roll_d20()
        ...
        result_text = f"🛡️ 豁免检定 - {self._get_ability_name_cn(ability_key)}\n"
        result_text += f"━━━━━━━━━━━━━━━━━━━━\n"
        result_text += f"检定者: {event.get_sender_name()}\n"
        result_text += f"━━━━━━━━━━━━━━━━━━━━\n"
        result_text += f"🎲 D20: {result.roll}\n"
        if char:
            result_text += f"📊 属性调整: {self._format_mod(mod)} (属性值{ability_score})\n"
        result_text += f"━━━━━━━━━━━━━━━━━━━━\n"
        result_text += f"📈 总计: {total}"

        yield event.plain_result(result_text)

    @dnd.command("attack", alias={"攻击"})
    @filter.event_message_type(event_filter.EventMessageType.GROUP_MESSAGE)
    async def dnd_attack(self, event: AstrMessageEvent, *, target: str = "敌人"):
        ...
        result_text += f"🎲 D20: {result.roll} + 攻击加值{attack_bonus:+d} = {result.total}\n"
        result_text += f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    async def dnd_spell(self, event: AstrMessageEvent, *, spell_name: str = ""):
        ...
        if not spell_name:
            yield event.plain_result("请提供法术名称，如: /dnd 法术 火球")
            return
