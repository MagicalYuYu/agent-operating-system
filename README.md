...
将 AOS 目录作为 Trae CODE 模式的工作区打开。Trae 会自动读取根目录的 `AGENTS.md`，AOS 的规则就开始生效了。

### 3. 配置 Trae Rules

在 Trae 设置 > 规则与记忆 > 规则 > 本地 中创建 3 条规则（详细内容见 [完整文档](#完整文档)）：

| 规则名 | 作用 |
|--------|------|
| aos-skill-rules | Skill 创建和调用规范 |
| aos-memory-rules | 记忆存储规范 |
| aos-consistency-guard | 文件修改时的一致性保障 |
...
| 功能 | 说明 |
|------|------|
| 网页知识入库 | 给一个 URL，自动提取结构化知识并存入参考知识库 |
| 存量内容迁移 | 把散落各处的历史项目、对话记录迁移到 AOS 体系 |
| 项目级配置 | 每个项目独立规则，AI 自动识别并加载 |
| 一致性自检 | 内置脚本验证文件引用、版本号、索引是否一致 |
| 可视化执行 | AI 每步操作都展示进度，不会后台静默运行 |
