...
| 路径 | 用途 | 允许的操作 |
|------|------|----------|
| `d:\AOS\` | 工作目录（含实际项目数据、凭据、记忆） | 日常开发、项目操作、Skill 执行、状态读写 |
| `d:\github\agent-operating-system\` | GitHub 上传纯净副本（仅框架文件） | git add/commit/push、GitHub Release 操作 |

### 路径识别规则

1. **当前工作目录为 `d:\AOS\`** → 工作模式，执行常规操作
2. **当前工作目录为 `d:\github\agent-operating-system\`** → 上传模式，仅执行 git 操作
3. **用户要求 GitHub 操作** → 切换到 `d:\github\agent-operating-system\` 执行
4. **用户要求开发/修改** → 在 `d:\AOS\` 执行，完成后通过同步脚本更新上传副本

### 同步规则

- 工作目录修改完成后，运行 `python d:\AOS\03_TOOLS\scripts\aos_sync_github.py` 同步到上传目录
